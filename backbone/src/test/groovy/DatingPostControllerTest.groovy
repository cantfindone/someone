...
class DatingPostControllerTest extends Specification {
    @Autowired
    MockMvc mvc
    @Shared
    def id;
    ...
    def "Save"() {
        def random = MockR.random(new TypeReference<DatingPostReq>() {
        })
        random.setId(null)
        String jsonString = com.alibaba.fastjson.JSON.toJSON(random)
        String res = mvc.perform(MockMvcRequestBuilders.post("/datingPost/")
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
                .content(jsonString)).andReturn().response.getContentAsString()
        println(res)
        id = JSON.parseObject(res, DatingPostReq.class).getId()
        expect:
        id != null
    }

    def "View"() {
        String res = mvc.perform(MockMvcRequestBuilders.get("/datingPost/".concat(id).concat("/view"))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON))
                .andReturn().response.getContentAsString()
    ...
    def "Like"() {
        String res = mvc.perform(MockMvcRequestBuilders.post("/datingPost/".concat(id).concat("/like"))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON))
                .andReturn().response.getContentAsString()
        def json = new JSONObject(res)
    ...
    def "Comment"() {
        def random = MockR.random(new TypeReference<DatingPostReq.Comment>() {
        })
        String jsonString = com.alibaba.fastjson.JSON.toJSON(random)
        String res = mvc.perform(MockMvcRequestBuilders.post("/datingPost/".concat(id).concat("/comment"))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
                .content(jsonString)).andReturn().response.getContentAsString()
        def json = new JSONObject(res)
    ...
    def "List"() {
        String res = mvc.perform(MockMvcRequestBuilders.get("/datingPost/list")
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON))
                .andReturn().response.getContentAsString()
        def json = new JSONObject(res)
